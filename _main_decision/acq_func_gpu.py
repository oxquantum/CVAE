...
def row_column(image_shape, patch_size):
    num_row = image_shape[0] // patch_size
    num_col = image_shape[1] // patch_size

    return num_row, num_col


def define_acqusition_function(sess, options):
    num_row, num_col = row_column(options.model_options.image_shape, options.DOE_options.patch_size)
    # Define likelihood, acquisition function
    pred_shape = (options.model_options.batch_size,) + options.model_options.image_shape + (1,)

    acq = AcquisitionFunc_gpu(options.tensorflow_options.device, sess, pred_shape, options.DOE_options.patch_size,
                              num_row, num_col, scale_L=options.DOE_options.distance_scale)
    return acq


class AcquisitionFunc_gpu(object):
    def __init__(self, device, sess, pred_shape, patch_size, num_row, num_col, scale_L=1.0):
    ...
    def build_graph(self):
        ...
        log_next, log_term, logZ_next = self.get_updated_prob(self.pred, self.observed, self.log_current_prob,
                                                              self.logZ)
    ...
    def get_score(self, obs_data_mask, predictions, log_current_prob, logZ):
        feed_dict = {self.pred: predictions, self.observed: obs_data_mask[..., 1:],
                     self.log_current_prob: log_current_prob, self.logZ: logZ}
